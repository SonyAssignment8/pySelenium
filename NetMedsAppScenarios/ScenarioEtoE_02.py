...
class NetMeds:
    ...
    def logintoAPP(self):
        ...
        self.familyareProduct()
    def familyareProduct(self):
        family_care_linkxpath=g.getJsonProperty("home","family_care")
        catagoriesxpath=g.getJsonProperty("catogories","baby_and_infants")
        sort_by_popularity=g.getJsonProperty("sorting","sort_by")
        addtocartbtnxpath=g.getJsonProperty("cart","addtocart")
        cart_btn_xpath=g.getJsonProperty("cart","cartcount")
        proceed_btn_xpath=g.getJsonProperty("cart","proceed")
        select_address=g.getJsonProperty("cart","seletaddress")
        exp_page_title=g.getJsonProperty("result","pageTitle")
        back_btn_xpath = "//a[text()='Back']"
        self.driver.find_element_by_xpath(family_care_linkxpath).click()
        self.driver.find_element_by_xpath(catagoriesxpath).click()
        self.driver.find_element_by_xpath(sort_by_popularity).click()
        self.driver.execute_script("window.scrollBy(0,250)")
        self.driver.find_element_by_xpath(addtocartbtnxpath).click()
        self.driver.find_element_by_xpath(cart_btn_xpath).click()
        self.driver.find_element_by_xpath(proceed_btn_xpath).click()
        actual=self.driver.find_element_by_xpath(select_address).text
        time.sleep(3)
        self.driver.find_element_by_xpath(back_btn_xpath).click()
